...
// src/services/articleService.js - ✅ FIXED VERSION
import { apiService } from "./apiService";
...
class ArticleService {
  async getArticles(params = {}) {
    try {
      ...
      // ✅ ADD /api prefix here
      const response = await apiService.get("/api/articles", cleanParams);
  ...
  async getArticleById(id) {
    try {
      // ✅ ADD /api prefix here
      const response = await apiService.get(`/api/articles/${id}`);
      console.log(`📖 Article ${id} response:`, response);
      return response;
    } catch (error) {
      console.error(`❌ Error fetching article ${id}:`, error);
      throw error;
  ...
  async getCategories() {
    try {
      // ✅ ADD /api prefix here
      const response = await apiService.get("/api/articles/categories");
      return response;
    } catch (error) {
      console.error("❌ Error fetching article categories:", error);
      throw error;
  ...
  async getFeaturedArticles() {
    try {
      // ✅ ADD /api prefix here
      const response = await apiService.get("/api/articles/featured");
      return response;
    } catch (error) {
      console.error("❌ Error fetching featured articles:", error);
      throw error;
  ...
  async createArticle(articleData) {
    try {
      ...
      if (articleData.imageFile || articleData.documentFile) {
        ...
        // ✅ ADD /api prefix here
        return await apiService.upload("/api/articles", formData);
      }

      // ✅ ADD /api prefix here
      return await apiService.post("/api/articles", articleData);
    } catch (error) {
      console.error("❌ Error creating article:", error);
      throw error;
    }
  ...
  async updateArticle(id, articleData) {
    try {
      ...
      if (articleData.imageFile || articleData.documentFile) {
        ...
        // ✅ ADD /api prefix here
        return await apiService.upload(`/api/articles/${id}`, formData);
      }

      // ✅ ADD /api prefix here
      return await apiService.put(`/api/articles/${id}`, articleData);
    } catch (error) {
  ...
  async deleteArticle(id) {
    try {
      console.log(`🗑️ Deleting article ${id}...`);
      // ✅ ADD /api prefix here
      return await apiService.delete(`/api/articles/${id}`);
    } catch (error) {
      console.error("❌ Error deleting article:", error);
      throw error;
    }
  ...
  async searchArticles(query, params = {}) {
    try {
      // ✅ ADD /api prefix here
      return await apiService.get("/api/articles/search", {
        q: query,
        ...params,
      });
    } catch (error) {
