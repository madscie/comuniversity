...
const ArticlesPage = () => {
  ...
  const fetchArticles = async () => {
    try {
      ...
      if (response.data.success) {
        // Convert tags from string to array and ensure proper data types
        const articlesWithArrayTags = response.data.data.articles.map(article => ({
          ...article,
          tags: article.tags ? 
        ...
        setArticles(articlesWithArrayTags);
        console.log("Articles with processed tags:", articlesWithArrayTags);
      } else {
        throw new Error(response.data.message || "Failed to fetch articles");
      }
    } catch (err) {
      ...
      {filteredArticles.length === 0 && !loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
          {filteredArticles.map((article) => (
            ...
            >
              {/* Article Image */}
              <div className="w-full h-32 sm:h-40 lg:h-48 rounded-lg mb-3 sm:mb-4 overflow-hidden bg-gray-100 dark:bg-gray-800 relative">
                {article.image_url &&
                article.image_url !== "" &&
                article.image_url !== "null" ? (
                  <img
              ...
              <div className="flex items-center text-xs sm:text-sm text-gray-500 dark:text-gray-400 space-x-2 sm:space-x-3 lg:space-x-4 mb-3 sm:mb-4">
                ...
                <span className="flex items-center">
                  <FiClock className="mr-1 h-3 w-3 sm:h-4 sm:w-4" /> {article.read_time} min read
                </span>
                <span className="flex items-center">
                  <FiEye className="mr-1 h-3 w-3 sm:h-4 sm:w-4" /> {article.views} views
                </span>
              </div>
