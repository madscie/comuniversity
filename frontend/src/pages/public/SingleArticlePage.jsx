// // frontend/pages/public/SingleArticlePage.js
...
//     console.log("Purchased articles from localStorage:", purchasedArticles);
//     console.log("Current article ID:", id);
//     console.log("Is current article purchased?", purchasedArticles.includes(id));
    
//     if (purchasedArticles.includes(id)) {
//       setHasPurchased(true);
//       setDebugInfo(`Article ${id} found in purchasedArticles`);
//     } else {
//       setDebugInfo(`Article ${id} NOT found in purchasedArticles`);
//     }
//   }, [id]);
...
//         console.log("Article data after processing:", articleData);
//         console.log("Article amount:", articleData.amount);
//         console.log("Is premium?", articleData.amount > 0);
...
//     if (method === 'stripe') {
//       // Use the exact format that works for Stripe
//       successUrl = `${window.location.origin}/payment-success?session_id={CHECKOUT_SESSION_ID}&article_id=${article._id || id}&provider=stripe`;
//     } else if (method === 'paypal') {
//       // Use the exact format that works for PayPal
//       successUrl = `${window.location.origin}/payment-success?order_id={ORDER_ID}&article_id=${article._id || id}&provider=paypal`;
//     }

//     const cancelUrl = `${window.location.origin}/payment-cancelled?article_id=${article._id || id}`;
...
//   const hasTextContent = article?.content && article.content.trim() !== "" && article.content !== "optional";
...
//     // FREE ARTICLES: Allow viewing/downloading without payment
//     if (article.amount === 0) {
//       console.log("Article is FREE (amount: 0)");
//       if (hasFileContent) {
//         return (
...
//                 onClick={() => {
//                   if (article.file_url) {
//                     window.open(getImageUrl(article.file_url), '_blank');
//                   }
//                 }}
//                 className="px-6 py-3"
//               >
...
//     if (article.amount > 0) {
//       console.log("Article is PREMIUM (amount > 0)");
//       console.log("Has purchased?", hasPurchased);
      
//       if (hasPurchased) {
//         console.log("User has PURCHASED this article");
...
//         );
//       } else {
//         console.log("User has NOT purchased this article - showing payment prompt");
//         // User hasn't paid - show payment prompt
//         return (
...
//                 Premium Article Locked
//               </h3>
//               <p className="text-gray-600 dark:text-gray-400 mb-6">
//                 Purchase this article to view or download the content
//               </p>
...
//                       <FiFileText className="h-16 w-16 text-gray-400 dark:text-gray-500 mx-auto mb-4" />
//                       <p className="text-gray-600 dark:text-gray-400">
//                         Document locked - purchase to view/download
//                       </p>
//                       <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
//                         File: {article.file_name || "Document"} ({article.file_size ? (article.file_size / 1024).toFixed(2) + ' KB' : 'Unknown size'})
//                       </p>
//                     </div>
//                   </div>
//                 </div>
...
//               <p>Is Premium: {article.amount > 0 ? 'YES' : 'NO'}</p>
//               <p>Debug: {debugInfo}</p>
//             </div>
//           </div>
//         )}
...
//             }`}>
//               {hasTextContent ? <FiType className="mr-1" /> : 
//                hasFileContent ? <FiFileText className="mr-1" /> : 
//                <FiBook className="mr-1" />}
//               {hasTextContent ? "Text Article" : 
...
//                     You must purchase this article to view or download the content
//                   </p>
//                 </div>
...
const SingleArticlePage = () => {
  ...
  const handlePayment = async (method) => {
    ...
    if (method === 'stripe') {
      // Stripe can handle the CHECKOUT_SESSION_ID placeholder
      successUrl = `${window.location.origin}/payment-success?session_id={CHECKOUT_SESSION_ID}&article_id=${article._id || id}&provider=stripe`;
    } else if (method === 'paypal') {
      // FIXED: PayPal CANNOT handle {ORDER_ID} placeholder - using simple URL
      successUrl = `${window.location.origin}/payment-success?article_id=${article._id || id}&provider=paypal`;
    }
