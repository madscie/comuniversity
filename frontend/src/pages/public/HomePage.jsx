...
const HomePage = () => {
  ...
  const { data: booksData, loading: booksLoading } = useApi(
  ...
  const totalBooks =
    booksData?.books?.length || booksData?.data?.books?.length || 2500;
  const totalCategories = categoriesData?.data?.length || 150;

  const stats = [
    { number: Math.floor(totalBooks / 100) * 100, label: "Books", suffix: "+" },
    { number: totalCategories, label: "Categories", suffix: "+" },
