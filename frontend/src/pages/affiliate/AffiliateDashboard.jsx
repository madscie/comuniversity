...
const AffiliateDashboard = () => {
  ...
  // Get affiliate status from Clerk user metadata
  const isAffiliate = user?.publicMetadata?.isAffiliate || false;
  const affiliateCode = user?.publicMetadata?.affiliateCode || user?.id?.slice(-8).toUpperCase();

  useEffect(() => {
    if (isSignedIn && isAffiliate) {
      loadAffiliateData();
    }
  }, [isSignedIn, isAffiliate]);

  const loadAffiliateData = async () => {
    setLoading(true);
    try {
      // TODO: Replace with actual API calls
      // const statsResponse = await fetch(`/api/affiliate/stats/${affiliateCode}`);
      // const statsData = await statsResponse.json();
      // setStats(statsData);
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600">
            Welcome back, {user?.firstName}! Track your referrals and earnings in real-time
          </p>
        </div>
