...
const DownloadHandler = ({ bookId, bookTitle, bookAuthor, price, format, onComplete, onCancel }) => {
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center">
          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${
            ...
            step === 'downloading' || step === 'complete' ? 'bg-green-600 text-white' : 'bg-gray-300 text-gray-600'
          }`}>
            1
          </div>
          <span className={`ml-2 text-sm font-medium ${
            step === 'payment' ? 'text-blue-600' : 
            step === 'downloading' || step === 'complete' ? 'text-green-600' : 'text-gray-500'
          }`}>
            Payment
          </span>
        </div>
        ...
        <div className="flex items-center">
          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${
            step === 'downloading' ? 'bg-blue-600 text-white' : 
            step === 'complete' ? 'bg-green-600 text-white' : 'bg-gray-300 text-gray-600'
          }`}>
            2
          </div>
          <span className={`ml-2 text-sm font-medium ${
            step === 'downloading' ? 'text-blue-600' : 
      ...
      <div className="min-h-64">
        ...
        {step === 'complete' && (
          <div className="text-center py-8">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">Download Complete!</h3>
            <p className="text-gray-600 mb-6">
              Your book "{bookTitle}" has been downloaded successfully in {format} format.
            </p>
